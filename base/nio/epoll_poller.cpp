...
// fd泄露引起普通用户访问无权限的文件
kish::epoll_poller::epoll_poller() : m_epoll_fd(epoll_create1(EPOLL_CLOEXEC)), m_query_list(KINIT_EVENT_SIZE) {}

void epoll_poller::addev(const handler_ptr &obs) {
    if (obs) {
        m_save_map.insert(std::make_pair(obs->fd(), obs));
...
handler_list &epoll_poller::poll(int timeout) {
    ...
    update_savemap();
    // 注意，maxevents不能为0!!!
    int readyn = ::epoll_wait(m_epoll_fd, &(*m_query_list.begin()), (int) m_query_list.size(), timeout);
    ...
    if (readyn > 0) {
        // todo: delete this print
        m_ret_list.reserve(readyn);
        for (int i = 0; i < readyn; ++i) {
            int curr_fd = m_query_list.at(i).data.fd;

#ifdef __DEBUG__
            printf("now to handle fd is %d:\n", curr_fd);
#endif

            uint32_t curr_ev = m_query_list.at(i).events;
            // 获取保存的epoll_handler指针
            if (m_save_map.find(curr_fd) != m_save_map.end()) {
                handler_ptr h = m_save_map.at(curr_fd);
                h->update_latest_events(curr_ev);
                m_ret_list.emplace_back(h);
            }
        }
    } else if (readyn < 0) {
...
void epoll_poller::update_savemap() {
    handler_map tmp = handler_map(m_save_map);
    for (auto &m:tmp) {
        if (m.second->dead()) {
...
//            epoll_ctl(m_epoll_fd, EPOLL_CTL_DEL, m.second->fd(), &ev);
            m_save_map.erase(m.first);
        }
    }
    tmp.clear();
    // todo: change to log
...
void epoll_poller::delev(const handler_ptr &obs) {
    if (obs && m_save_map.find(obs->fd()) != m_save_map.end()) {
        struct epoll_event ev{};
        ev.data.fd = obs->fd();
        ev.events = obs->events();
        epoll_ctl(m_epoll_fd, EPOLL_CTL_DEL, obs->fd(), &ev);
        m_save_map.erase(obs->fd());
    }
}
...
void epoll_poller::modev(const handler_ptr &obs) {
    if (obs && m_save_map.find(obs->fd()) != m_save_map.end()) {
        ...
        ev.data.fd = obs->fd();
        ev.events = obs->events();
        epoll_ctl(m_epoll_fd, EPOLL_CTL_ADD, obs->fd(), &ev);
    }
}
