...
struct llhttp_settings_s {
   ...
   * 0  - Proceed normally
   * 1  - Assume that request/response has no contents, and proceed to parsing the
   *      next message
   * 2  - Assume absence of contents (as above) and make `llhttp_execute()` return
   *      `HPE_PAUSED_UPGRADE`
   * -1 - Error
   * `HPE_PAUSED`
   */
 ...
 *
 * NOTE: if this function ever returns a non-pause psr_type error, it will continue
 ...
 * all incoming bytes as the part of the contents, up to the last byte of the
